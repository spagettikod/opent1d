...
type Connection struct {
	ID                 string             `json:"id"`
	PatienID           string             `json:"patientId"`
...
func FindEndpoint(email, password string) (Endpoint, error) {
	resp, err := callLogin(email, password, EndpointEU)
	if err != nil {
		return EndpointEU, fmt.Errorf("error while querying for endpoint: %w", err)
	}

	if resp.Data.Redirect {
		return Endpoints[resp.Data.Region], nil
	}
